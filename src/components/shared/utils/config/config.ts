...
export const PRODUCTION_DOMAINS = {
    COM: brandConfig?.platform?.hostname?.production?.com || 'deriv.com',
} as const;

// Staging app domains
export const STAGING_DOMAINS = {
    COM: brandConfig?.platform?.hostname?.staging?.com || 'staging.deriv.com',
} as const;

// WebSocket server URLs
export const WS_SERVERS = {
    STAGING: `${brandConfig?.platform?.derivws?.url?.staging || 'https://staging-api.derivws.com/trading/v1/'}options/ws/public`,
    PRODUCTION: `${brandConfig?.platform?.derivws?.url?.production || 'https://api.derivws.com/trading/v1/'}options/ws/public`,
} as const;

// =============================================================================
// Original App Constants (Preserved)
// =============================================================================
 ...
 */
export const getAuthenticatedSocketURL = async (): Promise<string> => {
    try {
        ...
        console.error('[DerivWS] Error in getAuthenticatedSocketURL:', error);
        return getDefaultServerURL();
    }
};

// Original synchronous getSocketURL for backwards compatibility
